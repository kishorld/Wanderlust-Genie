...
actor VideoWatchingCoordinator {
    ...
    @Published private(set) var sharedVideo: SpatialVideo?
    ...
    private func updateSharedVideo(video: SpatialVideo) {
        coordinatorDelegate.video = video
        sharedVideo = video
    }

    func coordinatePlayback(of video: SpatialVideo) async {
